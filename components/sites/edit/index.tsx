...
interface SitesEditFormProps extends FormComponentProps {
  siteId: string
}
...
const SitesEdit = (props: any) => {
  const { data, loading, error } = useQuery(FETCH_SITE_QUERY, {
    variables: { id: props.siteId },
    fetchPolicy: 'network-only',
  })

  const [updateSiteMutation] = useMutation(UPDATE_SITE_MUTATION)
    ...
    <Fragment>
      <div className="border border-solid border-gray-300 border-t-0 border-l-0 border-r-0 bg-white">
        <PageHeader
          ...
          title={<div style={{ margin: '12px 0 12px 12px' }}>Edit {name}</div>}
          backIcon={
...
export default Form.create<SitesEditFormProps>()(SitesEdit)
